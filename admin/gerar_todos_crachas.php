...
<style>
    .conteudo {
        ...
        height: 456px;
        text-align: center;
        /* Caminho da imagem - verifique se está correto em relação ao arquivo PHP */
        background-image: url('imagens/fundo.jpeg') !important; 
        background-size: cover;
        background-repeat: no-repeat;
        /* Propriedades cruciais para a impressão em 2026 */
    ...
    @media print {
        body { margin: 0; }
        .folha { page-break-after: always; } /* Garante que respeite a quebra de página */
    }
</style>
...
$militaresSelecionados = isset($_POST['militares_selecionados']) ? $_POST['militares_selecionados'] : array();

// Convert the array of IDs to a comma-separated string
$militaresSelecionadosString = implode(',', $militaresSelecionados);

// Use the selected IDs in your SQL query
$consulta = $mysqli->query('SELECT * FROM `militares` WHERE MIL_SIT = "1" AND MIL_CODIGO IN (' . $militaresSelecionadosString . ') ORDER BY `militares`.`END_COD` ASC');
...
while ($row = mysqli_fetch_array($consulta)) {
    ...
    echo "<tr><td background='imagens/fundo.jpeg'><center><br>";

    $ano = date("Y");
    echo "<br><br><br><B>3º GAC Ap - $ano </B><br>";
